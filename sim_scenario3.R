...
c <- -0.7 
d <- 0.7282 
a <- 0.75   # MA part negated to be consistent with +signs in the filter defn.
...
errAPTBATS_sim3 <- list()
seasAP <- 24 
for (i in c(1:31)){
  print(i)
  errAPTBATS_sim3[[i]] <- tbatsHighFreq(Yactual[(1:(366*24)) + ((i-1)*24)], 1, seasAP, 24, 365*24)
}
save.image(file = "simTBATS_AP_3.RData")

seasPA <- 4
errPATBATS_sim3 <- list()
for (i in c(1:31)){
  print(i)
  errPATBATS_sim3[[i]] <- tbatsHighFreq(Yt[(1:(96*366))+((i-1)*96)], 1, seasPA, 96, 365*96)
}
save(errPATBATS_sim3, file = "simTBATS_PA_3")

# Errors  
#load("simTBATS_AP_3.RData")
#load("simTBATS_PA_3")
errModPA <- errPATBATS_sim3[[1]]$model[1:(8760-24)]
errModAP <- errAPTBATS_sim3[[1]]$model[1:(8760-24)]
errHoldPA <- matrix(NA,31,24)
errHoldAP <- matrix(NA,31,24)
for (i in 1:31){
  errModPA <- c(errModPA,errPATBATS_sim3[[i]]$model[(8760-24+1):8760])
  errModAP <- c(errModAP,errAPTBATS_sim3[[i]]$model[(8760-24+1):8760])
  errHoldPA[i,] <- errPATBATS_sim3[[i]]$hold[1:24]
  errHoldAP[i,] <- errAPTBATS_sim3[[i]]$hold[1:24]
}
